...
using static Alchera.FaceSDK.RawImageCV;
using static FaceSDKSample.sample.CamCtx;
using static FaceSDKSample.sample.DemoClientServer.BestLivShots;
...
namespace FaceSDKSample.sample
{
    ...
    {
        public const string _REV = "260312100210";
        public const string _REF = "832";
        ...
        {
            public const int MAX_SHOT_CNT = 4;
        ...
        { 
            ...
            Mat mat_roi = new Mat(render_frame,
        ...
        protected override void Run()
        {
            ...
            while (IsRunning())
            {
                ...
                // > 아래는 테스트용 샘플값
                string API_ENC_TIMESTAMP = prop_settings.SVR_FACESVR_API_IMG_ENCRYPT_RSA_AES_TS; // ex: "25102714072455"
                ...
                {
                    prop_settings.SVR_FACESVR_API_URL = "https://stg-face.useb.co.kr";
                    Uri.TryCreate(prop_settings.SVR_FACESVR_API_URL, UriKind.Absolute, out api_svr_uri);

                    /*
                     {
                          // face.box.width < (input_image.width * config.min_face_size_rates)
                          // face.box.width > (input_image.width * config.max_face_size_rates)

                          // [https://stg-face.useb.co.kr/compare/validation-config]
                          "compare_validation": {
                            "mask_confidence": 0.5,
                            "max_face_size_rate": 1.0, "min_face_size_rate": 0.25,
                            "max_pitch_degree": 25.0,  "min_pitch_degree": -10.0,
                            "max_yaw_degree": 20.0,    "min_yaw_degree": -20.0,
                            "max_roll_degree": 20.0,   "min_roll_degree": -20.0,
                            "landmark_confidence": 0.8,
                            "f_quality_confidence": 63.96
                          },

                          // [https://stg-face.useb.co.kr/liveness/validation-config]                          
                          "liveness_validation": {
                            "mask_confidence": 0.5,
                ...
                if (cam.CaptureFrame())
                {
                    ...
                    int cap_img_bgr_width = cam_img_bgr.width;
                    int cap_img_bgr_height = cam_img_bgr.height;
                    byte[] cap_img_bgr_pixels = cam_img_bgr.pixels_bgr;
                    ...
                    if (prop_settings.UI_ETC_TOP_MSG_SHOW)
                    {
                        ...
                        {
                            ui_top_msg2 = "[capture src] rot=" + prop_settings.CAP_SRC_ROTATE
                                + ", w=" + ui_render_frame.Width + ", h=" + ui_render_frame.Height;

                            if (crop_cap_img_margin_by_aspect)
                            {
                                ui_top_msg3 += "> Cropped Src, Aspect Ratio(height/width): " + crop_img_aspect_h_w;
                            }
                            else
                            {
                                ui_top_msg3 = "> Use Original Cam Source (NO CROP LEFT/RIGHT)";
                            }
                        }
                        else
                        {
                            ui_top_msg2 = "[cam src] error! source is unavailable !";
                        }


                        CamCtx.DrawText(ui_render_frame, ui_top_msg2,
                             pt_liv_ui += new Point(0, 20), new Scalar(255, 0, 128), liv_ui_fnt_scl);
                        CamCtx.DrawText(ui_render_frame, ui_top_msg3,
                             pt_liv_ui += new Point(0, 20), new Scalar(255, 0, 255), liv_ui_fnt_scl * 0.9);

                        pt_liv_ui += new Point(0, 30);
                    }
                    else
                    {
                        ...
                        case DemoStage.DoPassiveLivenessAndCollectLivenessBestShot:
                            {
                                ...
                                if (check_minimum_img_siz_err_msg == "")
                                {
                                    ...
                                    if (prop_settings.UI_ETC_GUIDE_FACE_GUIDE_SHOW)
                                        CamCtx.RenderCicleHoleMat(ui_render_frame, 0.35f, 0.3f);
                                    ...
                                    if (liv_rst.is_liveness_ok_)
                                    {                                        
                                        CamCtx.DrawText(ui_render_frame, "Liveness Rst: OK", pt_liv_ui, new Scalar(0, 255, 0), liv_ui_fnt_scl);

                                        CamCtx.DrawText(ui_render_frame, "> liveness confidence=" + liv_rst.liveness_confidence_,
                                                pt_liv_ui += new Point(0, 30), new Scalar(255, 0, 0), liv_ui_fnt_scl);
                                        ...
                                        {
                                            throw new Exception("Liveness BestShot: INVALID BEST SHOT STATE !!!");
                                            //CamCtx.DrawText(ui_render_frame, "Liveness BestShot: INVALID BEST SHOT STATE !!!",
                                            //    pt_liv_ui += new Point(0, 30), new Scalar(0, 0, 255), liv_ui_fnt_scl + 0.2);
                                        }
                                        else
                                        {
                                            ...
                                            string bestshot_state = "Liveness BestShot: ";
                                            bestshot_state += ctx.best_liv_shots.shot_cnt + " Collected";

                                            CamCtx.DrawText(ui_render_frame, bestshot_state,
                                                pt_liv_ui += new Point(0, 30), new Scalar(128, 255, 128), liv_ui_fnt_scl);
                                    ...
                                    else
                                    {
                                        ...
                                        string liv_fail_msg = "Liveness Rst: Fail";
                                        liv_fail_msg += " , err=" + liv_rst.liveness_err_.ToString();

                                        CamCtx.DrawText(ui_render_frame, liv_fail_msg, pt_liv_ui, new Scalar(0, 0, 255), liv_ui_fnt_scl + 0.1);

                                        CamCtx.DrawText(ui_render_frame, liv_rst.last_err_msg1_, pt_liv_ui += new Point(0, 30), new Scalar(0, 0, 255), liv_ui_fnt_scl);
                                        CamCtx.DrawText(ui_render_frame, liv_rst.last_err_msg2_, pt_liv_ui += new Point(0, 30), new Scalar(0, 0, 255), liv_ui_fnt_scl);
                                        CamCtx.DrawText(ui_render_frame, liv_rst.last_err_msg3_, pt_liv_ui += new Point(0, 30), new Scalar(0, 0, 255), liv_ui_fnt_scl + 0.1);
                                    }

                                }
                                else
                                {
                                    CamCtx.DrawText(ui_render_frame, "Liveness Rst: Camera Minimum Src Size is Invalidl!", pt_liv_ui, new Scalar(0, 0, 255), liv_ui_fnt_scl + 0.1);
                                    CamCtx.DrawText(ui_render_frame, " > " + check_minimum_img_siz_err_msg, pt_liv_ui += new Point(0, 30), new Scalar(255, 0, 128), liv_ui_fnt_scl);
                                }
                        ...
                        case DemoStage.BGRImageLivenessMulitframe:
                            {
                                ...
                                if (prop_demo.LIV_BGR_IMG_LIV_API_TYPE == "ClientOnly")
                                {
                                    if (ctx.ui_finish_liveness_multiframe_task == null)
                                    {
                                        ...
                                        // use last shot face
                                        Face face = ctx.best_liv_shots.GetShot(3).detected_face;
                                        ...
                                        FaceSDK.Rst cli_liv_multi_rst 
                                            = fsdk.GetImgLivenessMulti(
                                                ctx.best_liv_shots[0].data_, ctx.best_liv_shots[0].width_, ctx.best_liv_shots[0].height_,
                                                ctx.best_liv_shots[1].data_, ctx.best_liv_shots[1].width_, ctx.best_liv_shots[1].height_,
                                                ctx.best_liv_shots[2].data_, ctx.best_liv_shots[2].width_, ctx.best_liv_shots[2].height_,
                                                ctx.best_liv_shots[3].data_, ctx.best_liv_shots[3].width_, ctx.best_liv_shots[3].height_,
                                                ref face);
                                    ...
                                    if (ctx.api_liveness_multiframe_task == null)
                                    {
                                        ...
                                        ctx.api_liveness_multiframe_task =
                                            ...
                                            RestConn _conn = new RestConn(_api_svr_url, USE_HTTP_PROXY, HTTP_PROXY_URL);
                                                ...
                                                ctx.best_liv_shots[0], ctx.best_liv_shots[1],
                                                ctx.best_liv_shots[2], ctx.best_liv_shots[3]};
                                ...
                                */
                                ...
                                if (!ctx.cmp_prepare_img_performed)
                                {
                                    ...
                                    if (prop_demo.COMPARE_API_TYPE == "FaceServer")
                                    {
                                        ...
                                        BlobImg cmp2_bgr = ctx.best_liv_shots[3]; // use lastest bestshot image
                                        Box cmp2_face_box = ctx.best_liv_shots.GetShot(3).detected_face.box;
                    ...
                    if(ctx.rec_ctx.rec_is_recording)
                    {
                        ...
                        {
                            CamCtx.DrawText(ui_render_frame, "<Rec!>",
